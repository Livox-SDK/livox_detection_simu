...
float nms_th)
{
  ...
  auto result = py::array_t<float>(obj_num*7);
  result.resize({obj_num,7});
  py::buffer_info buf_result = result.request();
  float* ptr_result = (float*)buf_result.ptr;

  for(int i=0;i<obj_num;i++)
  {
    ptr_result[i*7 + 0] = results[i].is_obj;
    ptr_result[i*7 + 1] = results[i].cls_num;
    ptr_result[i*7 + 2] = results[i].ry;
    ptr_result[i*7 + 3] = results[i].l;
    ptr_result[i*7 + 4] = results[i].w;
    ptr_result[i*7 + 5] = results[i].x;
    ptr_result[i*7 + 6] = results[i].z;
  }
